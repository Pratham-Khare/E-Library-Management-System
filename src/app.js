...
/* 1. Proxy trust --------------------------------------------------------------------------- Behind a reverse proxy, req.ip is the PROXY's address unless Express is told how many hops to unwind from X-Forwarded-For. Get this wrong and rate limiting breaks: too low and every client shares one bucket, too high and a client can spoof the header to mint a fresh bucket per request. */
...
/* 3. Security headers --------------------------------------------------------------------------- A JSON API needs a different CSP from an HTML app. The one exception is Swagger UI, which is real HTML with inline styles — so its CSP is relaxed only on its own route, below, rather than weakening the policy globally. */
app.use(
  helmet({
...
/* 4. CORS --------------------------------------------------------------------------- A function rather than a static list so the allow-list can be checked per request and a rejected origin can be LOGGED — otherwise a blocked frontend produces a browser console error with no server-side trace at all. */
app.use(
  cors({
...
/* 5. Compression --------------------------------------------------------------------------- Skipped for ebook streaming: PDFs and EPUBs are already compressed, so gzipping them burns CPU for nothing — and worse, it breaks HTTP Range requests, because byte offsets in the compressed stream do not correspond to offsets in the file the reader is trying to seek within. */
app.use(
  compression({
    filter: (req, res) => {
...
/* 6. Body parsing --------------------------------------------------------------------------- The size limit is a real defence: without it a single request can be sized to exhaust process memory. File uploads bypass these parsers entirely and are handled by multer with its own, larger caps. */
app.use(express.json({ limit: config.app.bodyLimit }));
app.use(express.urlencoded({ extended: true, limit: config.app.bodyLimit }));
app.use(cookieParser());

/* 7. Injection sanitising --------------------------------------------------------------------------- Must run AFTER the body parsers (there is no req.body before them) and BEFORE any route handler. See middlewares/sanitize.js. */
app.use(sanitizeRequest);

/* 8. HTTP parameter pollution --------------------------------------------------------------------------- `?role=MEMBER&role=ADMIN` arrives as an array. Code written expecting a string then behaves unpredictably. hpp keeps the last value — except for parameters that are legitimately repeatable. */
app.use(
  hpp({
...
/* 10. Global rate limiting --------------------------------------------------------------------------- A blanket ceiling. Tighter per-route limiters (auth, search, upload, AI) are applied at their own routes. */
app.use(globalRateLimiter());

/* 11. Static files --------------------------------------------------------------------------- Cover images and avatars only. EBOOKS ARE DELIBERATELY NOT SERVED HERE — they are the library's actual assets, and a static mount would let anyone with a URL download any book without borrowing it. Ebook access goes through a controller that verifies an active digital loan and then streams with Range support. */
const staticOptions = {
